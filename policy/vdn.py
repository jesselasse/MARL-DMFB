...
class VDN:
    def __init__(self, args, nargs):
        ...
        self.model_dir = args.model_dir + '/' + args.alg + '/fov{}/task{}/'.format(nargs.fov, nargs.task)
        # 如果存在模型则加载模型
        if self.args.load_model:
            path_rnn = self.model_dir + nargs.load_model_name + 'rnn_net_params.pkl'
            path_vdn = self.model_dir + nargs.load_model_name + 'vdn_net_params.pkl'
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):
        ...
        q_evals, q_targets = self.get_q_values(batch, max_episode_len, n_agents)
    ...
    def get_q_values(self, batch, max_episode_len, n_agents):
        episode_num = batch['o'].shape[0] # batch_size
        q_evals, q_targets = [], []
