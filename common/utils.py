...
def td_lambda_target(batch, max_episode_len, q_targets, args):
    ...
    '''
    n_step_return = torch.zeros(
        (episode_num, max_episode_len, args.n_agents, max_episode_len))
    for transition_idx in range(max_episode_len - 1, -1, -1):
        ...
        for n in range(1, max_episode_len - transition_idx):
            ...
            n_step_return[:, transition_idx, :, n] = (
                r[:, transition_idx] + args.gamma * n_step_return[:, transition_idx + 1, :, n - 1]) * mask[:, transition_idx]
    # --------------------------------------------------n_step_return---------------------------------------------------
    ...
    for transition_idx in range(max_episode_len):
        ...
        for n in range(1, max_episode_len - transition_idx):
            returns += pow(args.td_lambda, n - 1) * \
                n_step_return[:, transition_idx, :, n - 1]
